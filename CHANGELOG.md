...
Tooling and docs only. The wording of the dedication statement is unchanged from 2.0.0-beta.4. The generated files now differ from that release in their line breaks alone — not in a single word.

### Changed

- `statement.txt` holds one line per paragraph, so the dedication can be edited as prose rather than rewrapped by hand after every change. `make-variants.sh` now does the wrapping when it builds the four files, which is why those files differ from 2.0.0-beta.4 without any word having moved.
